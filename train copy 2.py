...
from torch.optim.lr_scheduler import ReduceLROnPlateau  # 匯入學習率調度器
...
def train_model(
    ...
    #scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=5)  # 目標: 最大化 Dice 分數
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                # 如果損失值為 NaN 則跳過該批次
                if torch.isnan(loss):
                    logging.error(f'發現 NaN 損失值，跳過該批次。')
                    continue
...
def get_args():
    parser = argparse.ArgumentParser(description='Train the Attention UNet on images and target masks')  # 修改這行
    parser.add_argument('--epochs', '-e', metavar='E', type=int, default=5, help='Number of epochs')
    parser.add_argument('--batch-size', '-b', dest='batch_size', metavar='B', type=int, default=2, help='Batch size')
    parser.add_argument('--learning-rate', '-l', metavar='LR', type=float, default=0.01, help='Learning rate', dest='lr')
    parser.add_argument('--load', '-f', type=str, default=False, help='Load model from a .pth file')
    parser.add_argument('--scale', '-s', type=float, default=0.5, help='Downscaling factor of the images')
    parser.add_argument('--validation', '-v', dest='val', type=float, default=15.0, help='Percent of the data that is used as validation (0-100)')
...
if __name__ == '__main__':
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # 設置設備
    logging.info(f'使用設備 {device}')

    model = AttentionUNet(n_channels=3, n_classes=args.classes, bilinear=args.bilinear)  # 修改這行
    model = model.to(memory_format=torch.channels_last).to(device=device)  # 將模型移至GPU

    logging.info(f'網絡:\n'
                 f'\t{model.n_channels} 個輸入通道\n'
                 f'\t{model.n_classes} 個輸出通道 (類別)\n'
                 f'\t{"Bilinear" if model.bilinear else "Transposed conv"} 上採樣')

    if args.load:
        state_dict = torch.load(args.load, map_location=device)
        del state_dict['mask_values']
        model.load_state_dict(state_dict)
        logging.info(f'從 {args.load} 載入模型')
    ...
    try:
        ...
        logging.error('檢測到 OutOfMemoryError！'
                      '啟用檢查點以減少內存使用，但這會減慢訓練速度。'
                      '考慮啟用 AMP (--amp) 進行快速和內存有效的訓練')
