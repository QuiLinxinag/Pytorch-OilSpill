...
def train_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Upload a small set of validation images (Input | GroundTruth | Prediction) to wandb
        try:
            model.eval()
            max_images = 4
            color_map = np.array([
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    for i in range(batch_size):
                        ...
                        h, w = gt.shape
                        gt_color = color_map[gt.reshape(-1)].reshape(h, w, 3)
                        pred_color = color_map[pred.reshape(-1)].reshape(h, w, 3)

                        # If input has 1 channel, replicate
                        if img.shape[2] == 1:
                            img = np.repeat(img, 3, axis=2)

                        # compose horizontally: input | gt | pred
                        pad = 5
                        canvas = np.zeros((h, w * 3 + pad * 2, 3), dtype=np.uint8)
                        canvas[:, :w] = img
                        canvas[:, w + pad:w + pad + w] = gt_color
                        canvas[:, 2 * w + 2 * pad:2 * w + 2 * pad + w] = pred_color

                        caption = f'Epoch {epoch} | idx {seen}'
                        val_images_to_log.append(wandb.Image(canvas, caption=caption))
                        seen += 1
                    if seen >= max_images:
                        break

            if len(val_images_to_log) > 0:
                experiment.log({'val_examples': val_images_to_log, 'epoch': epoch})
        except Exception as e:
            logging.warning(f'Failed to upload validation images to wandb: {e}')
...
if __name__ == '__main__':
    ...
    model = OnePeaceModel(n_channels=3, n_classes=5)
