...
def train_model(
        ...
        epochs: int = 1,
        batch_size: int = 1,
        learning_rate: float = 1e-5,  # 調整學習率
    ...
    dataset.transform = transform  # 這裡手動將 transform 加入 dataset
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    if check_for_nan(masks_pred, "masks_pred"):
                        continue
...
if __name__ == '__main__':
    ...
    model = AttentionUNet(n_channels=3, n_classes=args.classes, bilinear=args.bilinear)
    model.to(device=device)

    if args.load:
        model.load_state_dict(torch.load(args.load, map_location=device))
        logging.info(f'模型載入自 {args.load}')
