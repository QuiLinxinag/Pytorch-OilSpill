...
class AttentionGate(nn.Module):
    """Attention Gate 用於關注重要的特徵區域"""
    def __init__(self, F_g, F_l, F_int=None):
        super(AttentionGate, self).__init__()
        if F_int is None:
            F_int = F_l // 2  # 根據 F_l 來減少計算量
...
class MultiScaleAttentionGate(nn.Module):
    ...
    def forward(self, g, x):
        ...
        # 多尺度特徵融合
        x_scale = self.scale1(x)
        x_scale = self.scale2(x_scale)
        x_scale = self.scale3(x_scale)

        return x * psi * x_scale
...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels=None):
        ...
        self.double_conv = nn.Sequential(
            nn.Conv2d(in_channels, mid_channels, kernel_size=3, padding=1, groups=2, bias=False),  # 加入 Grouped Convolution
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_channels, out_channels, kernel_size=3, padding=1, groups=2, bias=False),  # Grouped Convolution
