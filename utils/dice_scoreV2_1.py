...
__all__ = [
    ...
    'update_confusion_matrix', 'confusion_metrics', 'pixel_accuracy', 'EMA'
]


# ---------------- Confusion Matrix Utilities -----------------
def update_confusion_matrix(cm: Tensor, preds: Tensor, targets: Tensor, num_classes: int) -> Tensor:
    """Update confusion matrix.
    cm: (C,C) tensor
    preds: (B,C,H,W) probs/logits or (B,H,W) labels
    targets: (B,H,W) labels
    """
    if preds.dim() == 4:
        preds = preds.argmax(dim=1)
    flat_p = preds.view(-1).to(torch.long)
    flat_t = targets.view(-1).to(torch.long)
    mask = (flat_t >= 0) & (flat_t < num_classes)
    flat_p = flat_p[mask]
    flat_t = flat_t[mask]
    idx = flat_t * num_classes + flat_p
    bins = torch.bincount(idx, minlength=num_classes * num_classes).reshape(num_classes, num_classes)
    cm += bins.to(cm.device)
    return cm


def confusion_metrics(cm: Tensor, epsilon: float = 1e-6) -> Dict[str, Tensor]:
    """Return per-class precision, recall, IoU, F1, support and macro averages."""
    tp = cm.diag().float()
    support = cm.sum(dim=1).float()
    pred_sum = cm.sum(dim=0).float()
    precision = tp / (pred_sum + epsilon)
    recall = tp / (support + epsilon)
    iou = tp / (support + pred_sum - tp + epsilon)
    f1 = 2 * tp / (pred_sum + support + epsilon)
    valid = support > 0
    macro = lambda x: (x[valid].mean() if valid.any() else torch.tensor(1.0, device=cm.device))
    return {
...
def pixel_accuracy(cm: Tensor) -> float:
    return (cm.diag().sum().float() / (cm.sum().float() + 1e-6)).item()
